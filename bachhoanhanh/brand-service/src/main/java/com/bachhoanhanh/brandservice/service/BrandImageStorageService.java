...
public class BrandImageStorageService {
    ...
    private static final String KEY_PREFIX = "brands/";
    ...
    public BrandImageStorageService(
            S3Client s3Client,
    ...
    public String upload(MultipartFile file) {
        ...
        try {
            ...
            throw new IllegalStateException("Could not read brand image", e);
        }
    }
    ...
    private void validate(MultipartFile file) {
        if (file.getSize() > MAX_FILE_SIZE_BYTES) {
            throw new IllegalArgumentException("Brand image must be 5MB or smaller");
        }
        String contentType = file.getContentType();
        if (contentType == null || !ALLOWED_CONTENT_TYPES.contains(contentType.toLowerCase(Locale.ROOT))) {
            throw new IllegalArgumentException("Brand image must be JPEG, PNG, WEBP, or GIF");
        }
    }
